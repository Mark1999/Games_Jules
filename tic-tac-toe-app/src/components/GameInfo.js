...
const GameInfo = ({ player1Name, player2Name, xIsNext, winner, isDraw, gameStatus, onPlayAgain }) => {
  let statusText;
  ...
  return (
    <div className="text-center my-4 sm:my-6 p-4 bg-white">
      ...
      {(gameStatus === 'winner' || gameStatus === 'draw') && (
        <button
          className="mt-3 px-6 py-3 bg-gradient-to-r from-blue-500 to-indigo-500 text-white font-semibold rounded-lg shadow-lg hover:from-blue-600 hover:to-indigo-600 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-opacity-75 transition-all duration-150 ease-in-out transform hover:scale-105"
          onClick={onPlayAgain}
        >
          Play Again
        </button>
